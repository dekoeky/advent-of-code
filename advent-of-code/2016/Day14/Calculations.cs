...
internal static class Calculations
{
    ...
    public static int Part2(string salt, int n)
    {
        int index = -1;
        var hashes = new Dictionary<int, string>();
        ...
        while (n > 0)
        {
            ...
            // Calculate the hash for this index
            var hash = hashes.GetOrCalculateAndInsert(index, () => StretchedHash(salt, index));
            ...
            for (var i = 0; i <= hash.Length - 3; i++)
            {
                ...
                for (int index2 = (index + 1); index2 <= (index + 1000); index2++)
                {
                    var hash2 = hashes.GetOrCalculateAndInsert(index2, () => StretchedHash(salt, index2));
    ...
    private static string CalculateHash(string salt, int index) => CalculateHash($"{salt}{index}");
    ...
    private static string StretchedHash(string salt, int index)
    {
        var hash = CalculateHash(salt, index);
